...
try {
    ...
    $stmt = $dbConnection->prepare("
        SELECT ts.strandName, COUNT(ss.userID) as student_count
    ...
    ");
    $stmt->execute();
    $strandEnrollmentData = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Total students
    $totalStudents = array_sum(array_column($strandEnrollmentData, 'student_count'));
...
<head>
    ...
    <title>Learnify Strand Enrollment Report</title>
        ...
        <div class="report-container" id="report">
            <div class="report-border">
                <div class="report-header">
                    <h1>Learnify Strand Enrollment Report</h1>
                </div>
                <div class="report-content">
                    <h2>Strand Enrollment</h2>
                    <table class="report-table">
                        <tr>
                            <th>Strand Name</th>
                            <th>Number of Students</th>
                        </tr>
                        <?php foreach ($strandEnrollmentData as $row): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($row['strandName']); ?></td>
                                <td><?php echo htmlspecialchars($row['student_count']); ?></td>
                            </tr>
                        <?php endforeach; ?>
                        <tr>
                            <td><strong>Total Students</strong></td>
                            <td><strong><?php echo htmlspecialchars($totalStudents); ?></strong></td>
                        </tr>
                    </table>
            ...
            downloadBtn.addEventListener('click', function() {
                ...
                try {
                    html2pdf()
                        .set({
                            margin: [3, 3, 3, 3],
                            filename: 'Learnify_Strand_Report_<?php echo $safeAdminName; ?>_<?php echo date('Ymd_His'); ?>.pdf',
