...
$weightsStmt = $dbConnection->prepare("SELECT quiz_weight, assignment_weight, attendance_weight FROM grading_weights WHERE teacherSectionID = :teacherSectionID");
$weightsStmt->bindParam(':teacherSectionID', $teacherSectionID, PDO::PARAM_INT);
$weightsStmt->execute();
$weights = $weightsStmt->fetch(PDO::FETCH_ASSOC);
$quizWeight = $weights ? $weights['quiz_weight'] : 40;
$assignmentWeight = $weights ? $weights['assignment_weight'] : 35;
...
// Fetch assignments
$assignmentListStmt = $dbConnection->prepare("SELECT assignmentID, title FROM assignments 
                                              WHERE teacherSectionID = :teacherSectionID AND archived = 0 
                                              AND YEAR(createdDate) = :selectedYear 
                                              ORDER BY createdDate");
$assignmentListStmt->bindParam(':teacherSectionID', $teacherSectionID, PDO::PARAM_INT);
$assignmentListStmt->bindParam(':selectedYear', $selectedYear, PDO::PARAM_INT);
$assignmentListStmt->execute();
$assignments = $assignmentListStmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch assignment scores
$assignmentStmt = $dbConnection->prepare("SELECT a_s.studentID, a_s.assignmentID, a_s.totalScore, a_s.maxScore
                                         FROM assignment_scores a_s
                                         JOIN assignments a ON a_s.assignmentID = a.assignmentID
                                         WHERE a.teacherSectionID = :teacherSectionID AND a_s.archived = 0
                                         AND YEAR(a_s.recordedDate) = :selectedYear");
$assignmentStmt->bindParam(':teacherSectionID', $teacherSectionID, PDO::PARAM_INT);
$assignmentStmt->bindParam(':selectedYear', $selectedYear, PDO::PARAM_INT);
$assignmentStmt->execute();
$assignmentScores = $assignmentStmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch attendance
$totalClassesStmt = $dbConnection->prepare("SELECT COUNT(DISTINCT attendanceDate) as totalClasses 
                                           FROM attendance 
                                           WHERE teacherSectionID = :teacherSectionID AND archived = 0 
                                           AND YEAR(attendanceDate) = :selectedYear");
$totalClassesStmt->bindParam(':teacherSectionID', $teacherSectionID, PDO::PARAM_INT);
$totalClassesStmt->bindParam(':selectedYear', $selectedYear, PDO::PARAM_INT);
$totalClassesStmt->execute();
$totalClasses = $totalClassesStmt->fetch(PDO::FETCH_ASSOC)['totalClasses'];

$attendanceStmt = $dbConnection->prepare("SELECT studentID, status 
                                         FROM attendance 
                                         WHERE teacherSectionID = :teacherSectionID AND archived = 0 
                                         AND YEAR(attendanceDate) = :selectedYear");
...
foreach ($students as $student) {
    ...
    foreach ($quizScores as $score) {
        if ($score['studentID'] == $studentID) {
            $quizCount++;
            $quizTotalScore += $score['totalScore'];
            $quizTotalMaxScore += $score['maxScore'];
            $percentage = $score['maxScore'] > 0 ? ($score['totalScore'] / $score['maxScore']) * 100 : 0;
            $studentQuizScores[$score['quizID']] = ['score' => round($percentage, 2)];
        }
    }
    $quizAvg = $quizTotalMaxScore > 0 ? ($quizTotalScore / $quizTotalMaxScore) * 100 : 0;
    ...
    foreach ($assignments as $assignment) {
        $studentAssignmentScores[$assignment['assignmentID']] = ['score' => 'N/A'];
    }
    foreach ($assignmentScores as $score) {
        if ($score['studentID'] == $studentID) {
            $assignmentCount++;
            $assignmentTotalScore += $score['totalScore'];
            $assignmentTotalMaxScore += $score['maxScore'];
            $percentage = $score['maxScore'] > 0 ? ($score['totalScore'] / $score['maxScore']) * 100 : 0;
            $studentAssignmentScores[$score['assignmentID']] = ['score' => round($percentage, 2)];
        }
    }
    $assignmentAvg = $assignmentTotalMaxScore > 0 ? ($assignmentTotalScore / $assignmentTotalMaxScore) * 100 : 0;
    ...
    $lateEquiv = floor($late / 3);
    $adjustedAbsent = $absent + $lateEquiv;
    $attendancePercentage = $totalClasses > 0 ? (($totalClasses - $adjustedAbsent) / $totalClasses) * 100 : 0;

    // Final grade
    $finalGrade = ($quizAvg * ($quizWeight / 100)) + ($assignmentAvg * ($assignmentWeight / 100)) + ($attendancePercentage * ($attendanceWeight / 100));
    ...
    $allData[$studentID] = [
        ...
        'quizAvg' => round($quizAvg, 2),
        'assignmentAvg' => round($assignmentAvg, 2),
...
$yearStmt = $dbConnection->prepare("
    SELECT DISTINCT year FROM (
        SELECT YEAR(qs.recordedDate) AS year FROM quiz_scores qs
        JOIN quizzes q ON qs.quizID = q.quizID
        WHERE q.teacherSectionID = :teacherSectionID AND qs.archived = 0
        UNION
        SELECT YEAR(a_s.recordedDate) AS year FROM assignment_scores a_s
        JOIN assignments a ON a_s.assignmentID = a.assignmentID
        WHERE a.teacherSectionID = :teacherSectionID AND a_s.archived = 0
        UNION
...
<head>
    ...
    <title>Learnify - Final Grades</title>
    <style>
        :root {
        ...
        @media print {
            ...
            .tab-content:not(#grade-report-tab) {
    ...
    <section id="content">
        ...
        <main>
            <div class="head-title">
                <div class="left">
                    ...
                    <ul class="breadcrumb">
                        ...
                        <li><i class='bx bx-chevron-right'></i></li>
                        <li><a class="active" href="#">Final Grades</a></li>
                    </ul>
                </div>
            ...
            <div class="grade-rate-container">
                <div class="grade-rate-header">
                    <h1 class="grade-rate-title"><?php echo htmlspecialchars($sectionName . ' - ' . $subjectName); ?></h1>
                    <p>Quiz Weight: <span><?php echo $quizWeight; ?>% </span><br>
                       Assignment Weight: <span><?php echo $assignmentWeight; ?>%</span><br>
                       Attendance Weight: <span><?php echo $attendanceWeight; ?>%</span></p>
                </div>
                ...
                <div class="tab-buttons">
                    <button class="tab-button active" data-tab="grade-report-tab">Grade Report</button>
                    <button class="tab-button" data-tab="help-tab">Help/Guide</button>
                </div>

                <div id="grade-report-tab" class="tab-content active">
                    <form class="filter-container" method="GET">
                        ...
                        <?php if ($selectedYear != 2025 || $searchQuery): ?>
                            <a href="all_rate.php?teacherSectionID=<?php echo $teacherSectionID; ?>&sectionID=<?php echo $sectionID; ?>&subjectID=<?php echo $subjectID; ?>" class="clear-btn">Clear Filters</a>
                        <?php endif; ?>
                    </form>
                    ...
                    <div class="report-container" id="grade-report">
                        <div class="report-header">
                            <h1>Learnify Final Grade Report (<?php echo $selectedYear; ?><?php echo $searchQuery ? ' - Filtered: ' . htmlspecialchars($searchQuery) : ''; ?>)</h1>
                        </div>
                        <table class="grade-table">
                            <thead>
                                <tr>
                                    ...
                                    <?php foreach ($quizzes as $quiz): ?>
                                        <th class="item-title" title="<?php echo htmlspecialchars($quiz['title']); ?>">
                                            <?php echo htmlspecialchars(substr($quiz['title'], 0, 15)) . (strlen($quiz['title']) > 15 ? '...' : ''); ?>
                                        </th>
                                    <?php endforeach; ?>
                                    <?php foreach ($assignments as $assignment): ?>
                                        <th class="item-title" title="<?php echo htmlspecialchars($assignment['title']); ?>">
                            ...
                            <tbody>
                                ...
                                <?php foreach ($allData as $studentID => $data): ?>
                                    <tr>
                                        ...
                                        <?php foreach ($assignments as $assignment): ?>
                                            <td>
                                                <?php echo $data['assignmentScores'][$assignment['assignmentID']]['score'] == 'N/A' ? 'N/A' : $data['assignmentScores'][$assignment['assignmentID']]['score'] . '%'; ?>
                                            </td>
                                        <?php endforeach; ?>
                                        <td><?php echo $data['quizCount']; ?></td>
                                        <td><?php echo $data['assignmentCount']; ?></td>
                                        <td><strong style="color: #28a745"><?php echo $data['quizAvg']; ?>%</strong></td>
                                        <td><strong style="color: #28a745"><?php echo $data['assignmentAvg']; ?>%</strong></td>
                                        <td><strong style="color: #28a745"><?php echo $data['attendancePercentage']; ?>%</strong></td>
                                        <td><strong style="color: #007bff"><?php echo $data['finalGrade']; ?>%</strong></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                    ...
                    <?php if ($totalPages > 1): ?>
                        <div class="pagination">
                            <?php
                            $baseUrl = "all_rate.php?teacherSectionID=$teacherSectionID&sectionID=$sectionID&subjectID=$subjectID&year=$selectedYear" . ($searchQuery ? "&search=" . urlencode($searchQuery) : '');
                            $prevPage = $page - 1;
                            $nextPage = $page + 1;
                            ?>
                ...
                <div id="help-tab" class="tab-content">
                    <div class="help-content">
                        <h3>Final Grade Computation Guide</h3>
                        <p>The final grade is calculated based on the following formulas and weights:</p>
                        <ul>
                            <li><strong>Quiz Average (%)</strong>: (Sum of Approved Quiz Scores ÷ Sum of Maximum Possible Scores) × 100. Weight: <?php echo $quizWeight; ?>%.</li>
                            <li><strong>Assignment Average (%)</strong>: (Sum of Assignment Scores ÷ Sum of Maximum Possible Scores) × 100. Weight: <?php echo $assignmentWeight; ?>%.</li>
                            <li><strong>Attendance (%)</strong>: ((Total Classes - Adjusted Absences) ÷ Total Classes) × 100. Weight: <?php echo $attendanceWeight; ?>%.</li>
                            <li><strong>Final Grade (%)</strong>: (QuizAvg × <?php echo $quizWeight/100; ?>) + (AssignmentAvg × <?php echo $assignmentWeight/100; ?>) + (Attendance% × <?php echo $attendanceWeight/100; ?>), rounded to the nearest integer.</li>
                        </ul>
                        <h3>Computation Steps</h3>
                        <p><strong>Step 1: Quiz Average</strong></p>
                        <p>For each approved quiz: (totalScore ÷ maxScore) × 100. Sum all scores, divide by sum of max scores, multiply by 100.</p>
                        <p><strong>Step 2: Assignment Average</strong></p>
                        <p>For each scored assignment: (totalScore ÷ maxScore) × 100. Sum all scores, divide by sum of max scores, multiply by 100.</p>
                        <p><strong>Step 3: Attendance Percentage</strong></p>
                        <p>Count Present (1 point), Late (0.5 point), Absent (0 point). Convert lates to absents (3 lates = 1 absent). Adjusted Absences = Absents + floor(Lates/3). Attendance % = ((Total Classes - Adjusted Absences) ÷ Total Classes) × 100.</p>
                        <p><strong>Step 4: Final Grade</strong></p>
                        <p>Combine averages with weights: (QuizAvg × <?php echo $quizWeight/100; ?>) + (AssignmentAvg × <?php echo $assignmentWeight/100; ?>) + (Attendance% × <?php echo $attendanceWeight/100; ?>).</p>
                        <h3>Notes</h3>
                        <ul>
                            <li>Only approved, non-archived quizzes and non-archived assignments/attendance are included.</li>
                            <li>If a student has no scores for a quiz/assignment, it is shown as <strong>N/A</strong>.</li>
                            <li>If no scores exist for quizzes/assignments or no attendance records, the respective average is <strong>0%</strong>.</li>
                            <li>Final grade is rounded to the nearest integer.</li>
                        </ul>
                    </div>
                </div>
            </div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (downloadBtn) {
                downloadBtn.addEventListener('click', function() {
                    ...
                    if (!element) {
                        alert('Error: Grade report content not found.');
                        return;
                    }
                    ...
                    html2pdf()
                        .set({
                            margin: [5, 5, 5, 5],
                            filename: `Learnify_Final_Grade_${safeProfessorName}_${timestamp}.pdf`,
