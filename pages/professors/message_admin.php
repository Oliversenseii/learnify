...
try {
    // Get user details
    $stmt = $dbConnection->prepare("SELECT firstName, userType, image FROM users WHERE userID = :userID AND userType = 'Professor'");
    ...
    if (!$user) {
        error_log("User not found or not a Professor: userID=$userID");
        header("Location: ../../index.php");
        exit;
    }
    ...
    if (isset($_POST['submitMessage']) && isset($_POST['recipientID'])) {
        ...
        if ($recipientID && ($messageContent || $attachmentPath) && !isset($_SESSION['error_message'])) {
            ...
            $checkAdminStmt = $dbConnection->prepare("
                SELECT userID 
                FROM users 
                WHERE userID = :recipientID AND userType = 'Admin' AND archived = 0
            ");
            $checkAdminStmt->bindParam(':recipientID', $recipientID, PDO::PARAM_INT);
            ...
            if ($checkAdminStmt->rowCount() > 0) {
                ...
                $_SESSION['error_message'] = "Invalid recipient. Please select a valid Admin.";
            }
        } else {
            $_SESSION['error_message'] = "Either message content or an attachment is required.";
        }
        header("Location: message_admin.php" . (isset($_GET['adminID']) ? "?adminID=" . $_GET['adminID'] : ""));
        exit;
    }
    ...
    // Fetch admins for selection (include email and image)
    $adminStmt = $dbConnection->prepare("
        SELECT userID, firstName, lastName, CONCAT(firstName, ' ', lastName) AS adminName, email, image
        FROM users
        WHERE userType = 'Admin' AND archived = 0
        ORDER BY firstName
    ");
    if (!$adminStmt->execute()) {
        throw new PDOException("Failed to fetch admins");
    }
    $admins = $adminStmt->fetchAll(PDO::FETCH_ASSOC);

    // Fetch selected admin details
    $selectedAdmin = null;
    $selectedAdminID = isset($_GET['adminID']) ? filter_var($_GET['adminID'], FILTER_VALIDATE_INT) : null;
    if ($selectedAdminID) {
        $selectedAdminStmt = $dbConnection->prepare("
            SELECT userID, firstName, lastName, email, image
            FROM users
            WHERE userID = :adminID AND userType = 'Admin' AND archived = 0
        ");
        $selectedAdminStmt->bindParam(':adminID', $selectedAdminID, PDO::PARAM_INT);
        if ($selectedAdminStmt->execute()) {
            $selectedAdmin = $selectedAdminStmt->fetch(PDO::FETCH_ASSOC);
        }
    }

    // Fetch private messages for a specific admin if selected
    $messages = [];
    if ($selectedAdminID) {
        $messageStmt = $dbConnection->prepare("
            ...
            WHERE am.archived = 0 
            AND ((am.sender_id = :userID AND am.recipient_id = :adminID) OR (am.sender_id = :adminID AND am.recipient_id = :userID))
            ORDER BY am.created_at ASC
        ");
        $messageStmt->bindParam(':userID', $userID, PDO::PARAM_INT);
        $messageStmt->bindParam(':adminID', $selectedAdminID, PDO::PARAM_INT);
    ...
    // Categorize attachments for the selected admin
    $attachments = $selectedAdminID ? categorizeAttachments($messages) : ['media' => [], 'files' => []];

    // Notification logic: Fetch new private messages from admins
    ...
    error_log("Database Error in message_admin.php: " . $e->getMessage() . " at line " . $e->getLine());
...
<head>
    ...
    <title>Learnify - Messages to Admins</title>
    <style>
        :root {
            --primary: #1a73e8;
            --secondary: #f1f3f4;
        ...
        .admin-item.active .admin-info h4 {
            color: #007bff;
        }
            ...
            <div class="messenger-container">
                <div class="admin-list">
                    <?php if (!empty($admins)): ?>
                        <?php foreach ($admins as $admin): ?>
                            <?php
                                ...
                                foreach ($newMessages as $newMessage) {
                                    if ($newMessage['userID'] == $admin['userID']) {
                                        $newMessageCount++;
                                    }
                                }
                            ?>
                            <a href="message_admin.php?adminID=<?php echo $admin['userID']; ?>" 
                               class="admin-item <?php echo $selectedAdminID == $admin['userID'] ? 'active' : ''; ?>">
                                <img src="<?php echo htmlspecialchars($admin['image'] ?: './img/noprofile.png'); ?>" alt="Admin Profile">
                                <div class="admin-info">
                                    <h4><?php echo htmlspecialchars(ucwords($admin['adminName'])); ?></h4>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <p class="no-messages">No Admins available.</p>
                    <?php endif; ?>
                </div>
                ...
                <div class="chat-area">
                    <?php if ($selectedAdmin): ?>
                        <div class="chat-content">
                            <div class="chat-header">
                                <img src="<?php echo htmlspecialchars($selectedAdmin['image'] ?: './img/noprofile.png'); ?>" alt="Selected Admin Profile">
                                <h3><?php echo htmlspecialchars(ucwords($selectedAdmin['firstName'] . ' ' . $selectedAdmin['lastName'])); ?></h3>
                            </div>
                            <div class="chat-messages" id="chat-messages">
                                <?php if (!empty($messages)): ?>
                                    <?php foreach ($messages as $message): ?>
                                        <div class="message <?php echo $message['sender_id'] == $userID ? 'sent' : 'received'; ?>">
                                            <div class="message-content <?php echo $message['sender_id'] == $userID ? 'sent' : 'received'; ?>">
                                    ...
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <p class="no-messages">No messages with this admin.</p>
                                <?php endif; ?>
                            </div>
                            <form class="message-form" method="POST" enctype="multipart/form-data">
                                <input type="hidden" name="recipientID" value="<?php echo $selectedAdminID; ?>">
                    ...
                    <?php else: ?>
                        <p class="no-messages">Select an admin to start messaging.</p>
                    <?php endif; ?>
                </div>
            </div>
