...
$weightsStmt = $dbConnection->prepare("SELECT assignment_weight FROM grading_weights WHERE teacherSectionID = :teacherSectionID");
...
// Fetch assignments for the given teacherSectionID and year
try {
    ...
    error_log("Database Error: " . $e->getMessage());
    $_SESSION['error_message'] = "Failed to fetch assignments: " . htmlspecialchars($e->getMessage());
    header("Location: professorDash.php");
    exit;
}
...
try {
    ...
    error_log("Database Error: " . $e->getMessage());
    $_SESSION['error_message'] = "Failed to fetch assignment scores: " . htmlspecialchars($e->getMessage());
    header("Location: professorDash.php");
    exit;
}

// Compute assignment data
$assignmentData = [];
foreach ($students as $student) {
    ...
    $averageAssignmentScore = $totalMaxScore > 0 ? ($totalScore / $totalMaxScore) * 100 : 0;
    ...
    $assignmentData[$studentID] = [
        ...
        'averageAssignmentScore' => round($averageAssignmentScore, 2),
        'assignmentScores' => $studentAssignmentScores
    ];
}

// Fetch available years for filter
$yearStmt = $dbConnection->prepare("SELECT DISTINCT YEAR(a_s.recordedDate) AS year 
                                   FROM assignment_scores a_s
                                   JOIN assignments a ON a_s.assignmentID = a.assignmentID
                                   WHERE a.teacherSectionID = :teacherSectionID AND a_s.archived = 0 
                                   ORDER BY year DESC");
...
<head>
    ...
    <title>Learnify - Assignment Rate</title>
    <style>
        :root {
        ...
        .assignment-rate-header p {
            font-size: clamp(1rem, 3vw, 1.5rem);
            color: var(--dark);
        }

        .assignment-rate-header p span {
        ...
        @media print {
            ...
            .tab-content:not(#assignment-report-tab) {
    ...
    <section id="content">
        ...
        <main>
            <div class="head-title">
                <div class="left">
                    <h1>Assignment Rate</h1>
                    <ul class="breadcrumb">
                        <li><a href="#">Home</a></li>
                        <li><i class='bx bx-chevron-right'></i></li>
                        <li><a href="#">Dashboard</a></li>
                        <li><i class='bx bx-chevron-right'></i></li>
                        <li><a class="active" href="#">Assignment Rate</a></li>
                    </ul>
                </div>
            ...
            <div class="assignment-rate-container">
                <div class="assignment-rate-header">
                    <h1 class="assignment-rate-title"><?php echo htmlspecialchars($sectionName . ' - ' . $subjectName); ?></h1>
                    <p>Assignment Weight: <span><?php echo $assignmentWeight; ?>%</span></p>
                </div>
                ...
                <div class="tab-buttons">
                    <button class="tab-button active" data-tab="assignment-report-tab">Assignment Report</button>
                    <button class="tab-button" data-tab="help-tab">Help/Guide</button>
                </div>

                <div id="assignment-report-tab" class="tab-content active">
                    <form class="filter-container" method="GET">
                        ...
                        <?php if ($selectedYear != 2025 || $searchQuery): ?>
                            <a href="assignment_rate.php?teacherSectionID=<?php echo $teacherSectionID; ?>&sectionID=<?php echo $sectionID; ?>&subjectID=<?php echo $subjectID; ?>" class="clear-btn">Clear Filters</a>
                        <?php endif; ?>
                    </form>
                    ...
                    <div class="report-container" id="assignment-report">
                        <div class="report-header">
                            <h1>Learnify Assignment Rate Report (<?php echo $selectedYear; ?><?php echo $searchQuery ? ' - Filtered: ' . htmlspecialchars($searchQuery) : ''; ?>)</h1>
                        </div>
                        <table class="assignment-table">
                            <thead>
                                <tr>
                                    ...
                                    <?php foreach ($assignments as $assignment): ?>
                                        <th class="assignment-title" title="<?php echo htmlspecialchars($assignment['title']); ?>">
                            ...
                            <tbody>
                                ...
                                <?php foreach ($assignmentData as $studentID => $data): ?>
                                    <tr>
                                        ...
                                        <td><strong style="color: #28a745"><?php echo $data['averageAssignmentScore']; ?>%</strong></td>
                                        <td><?php echo $assignmentWeight; ?>%</td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                    ...
                    <?php if ($totalPages > 1): ?>
                        <div class="pagination">
                            <?php
                            $baseUrl = "assignment_rate.php?teacherSectionID=$teacherSectionID&sectionID=$sectionID&subjectID=$subjectID&year=$selectedYear" . ($searchQuery ? "&search=" . urlencode($searchQuery) : '');
                            $prevPage = $page - 1;
                            $nextPage = $page + 1;
                            ?>
                ...
                <div id="help-tab" class="tab-content">
                    <div class="help-content">
                        <h3>Assignment Rate Computation Guide</h3>
                        <p>The assignment rate is calculated based on the following formulas:</p>
                        <ul>
                            <li><strong>Per Assignment Score (%)</strong>: (totalScore ÷ maxScore) × 100 for each scored assignment.</li>
                            <li><strong>Average Assignment Score (%)</strong>: (Sum of all Assignment Scores ÷ Sum of Maximum Possible Scores) × 100.</li>
                            <li>Only non-archived assignment scores are included in the calculation.</li>
                            <li>If a student has no scored assignments, their average is <strong>0%</strong>.</li>
                            <li>If a student has not submitted or scored an assignment, the score is displayed as <strong>N/A</strong>.</li>
                        </ul>
                        <h3>Computation Steps</h3>
                        <p><strong>Step 1: Per Assignment Score</strong></p>
                        <p>For each assignment, calculate: (totalScore ÷ maxScore) × 100.</p>
                        <p><strong>Step 2: Sum Assignment Scores</strong></p>
                        <p>Add up the <strong>totalScore</strong> for all scored assignments for a student.</p>
                        <p><strong>Step 3: Sum Maximum Scores</strong></p>
                        <p>Add up the <strong>maxScore</strong> for all scored assignments for a student.</p>
                        <p><strong>Step 4: Calculate Average</strong></p>
                        <p>Average Assignment Score (%) = (Total Assignment Scores ÷ Total Maximum Scores) × 100</p>
                        <h3>Assignment Weight</h3>
                        <p>The admin-assigned weight for assignments in the final grade calculation is <strong><?php echo $assignmentWeight; ?>%</strong>.</p>
                        <p><em>Note: The assignment average percentage shown in the report is used in the final grade computation alongside attendance and quiz averages, weighted as defined by the admin.</em></p>
                    </div>
                </div>
            </div>
        </main>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (downloadBtn) {
                downloadBtn.addEventListener('click', function() {
                    const element = document.getElementById('assignment-report');
                    if (!element) {
                        alert('Error: Assignment report content not found.');
                        return;
                    }
                    ...
                    html2pdf()
                        .set({
                            margin: [5, 5, 5, 5],
                            filename: `Learnify_Assignment_Rate_${safeProfessorName}_${timestamp}.pdf`,
