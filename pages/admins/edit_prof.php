...
require_once './helpers/decrypt.php';
require_once './check_status.php';

// Check if userID is provided in the URL
if (!isset($_GET['userID'])) {
    header('Location: data_professor.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($updateStmt->execute()) {
        $_SESSION['success_message'] = "Teacher user updated successfully!";
    } else {
        $_SESSION['error_message'] = "Error updating teacher user. Please try again.";
    }
	...
	<section id="content">
		...
		<main>
			<div class="head-title">
				<div class="left">
					<h1><?php echo htmlspecialchars($professor['lastName']); ?>'s Data</h1>
					<ul class="breadcrumb">
						<li>
							<a href="#">Home</a>
						</li>
						...
						<li>
							<a href="edit_admin.php">Data</a>
						</li>
                        <li><i class='bx bx-chevron-right' ></i></li>
						<li>
							<a class="active" href="edit_prof.php?userID=<?php echo $professor['userID']; ?>">Edit</a>
						</li>
					</ul>
				</div>
                <a href="data_professor.php" class="back-button">
			...
			<!-- Table to display administrators -->
			  ...
			  <form method="POST" class="edit-form">
                <div class="form-group">
                    ...
                    <input type="text" name="firstName" value="<?php echo htmlspecialchars($professor['firstName']); ?>" required>
                </div>
                <div class="form-group">
                    <label for="lastName">Last Name</label>
                    <input type="text" name="lastName" value="<?php echo htmlspecialchars($professor['lastName']); ?>" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="contactNumber" value="<?php echo htmlspecialchars($professor['contactNumber']); ?>" required>
                </div>
                <div class="form-group">
                    <label for="status">Status</label>
                    <select name="status" required>
                        <option value="Active" <?php echo $professor['status'] === 'Active' ? 'selected' : ''; ?>>Active</option>
                        <option value="Inactive" <?php echo $professor['status'] === 'Inactive' ? 'selected' : ''; ?>>Inactive</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="birthday">Birthday</label>
                    <input type="date" name="birthday" id="birthday" value="<?php echo htmlspecialchars($professor['birthday']); ?>" required>
                </div>
