...
$offset = ($page - 1) * $professors_per_page;

// Build SQL query for counting total professors
...
if ($search !== '') {
    $count_sql .= " AND (firstName LIKE :search OR lastName LIKE :search OR email LIKE :search OR LOWER(sex) LIKE :search)";
    $count_params[':search'] = "%" . strtolower($search) . "%";
}
...
try {
    ...
    $count_stmt->execute();
    $total_professors = $count_stmt->fetch(PDO::FETCH_ASSOC)['total'];
    $total_pages = ceil($total_professors / $professors_per_page);
...
// Build SQL query for fetching paginated professors
...
if ($search !== '') {
    $sql .= " AND (firstName LIKE :search OR lastName LIKE :search OR email LIKE :search OR LOWER(sex) LIKE :search)";
...
try {
    ...
    $stmt->bindValue(':limit', $professors_per_page, PDO::PARAM_INT);
    ...
    <section id="content">
        ...
        <main>
            <div class="head-title">
                <div class="left">
                    <h1>Teachers<?php echo ($valid_sex || $valid_age_group) ? ' (' . ($valid_sex ? ucfirst($valid_sex) : '') . ($valid_sex && $valid_age_group ? ', ' : '') . ($valid_age_group ? 'Age ' . $valid_age_group : '') . ')' : ''; ?></h1>
                    <ul class="breadcrumb">
                        <li><a href="#">Home</a></li>
                        <li><i class='bx bx-chevron-right'></i></li>
                        <li><a class="active" href="data_professor.php">Teachers</a></li>
                    </ul>
                </div>
            ...
            <?php if ($valid_sex || $valid_age_group): ?>
                <div class="filter-notification">
                    ...
                    <?php echo $valid_age_group ? 'Age ' . $valid_age_group : ''; ?> Teachers
                </div>
            <?php endif; ?>
            ...
            <form method="GET" class="search-form">
                ...
                <?php if (!empty($search) || $valid_sex || $valid_age_group): ?>
                    <a href="data_professor.php" class="clear-btn">Clear</a>
                <?php endif; ?>
                <?php if ($valid_sex): ?>
            ...
            <div class="admin-table">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($professor['image']): ?>
                                            <img src="<?php echo htmlspecialchars($professor['image']); ?>" alt="Profile Image" class="profile-img">
                                        <?php else: ?>
                                            <img src="./img/noprofile.png" alt="No Profile Image" class="profile-img">
                                        <?php endif; ?>
                                    </td>
                                    <td><?php echo htmlspecialchars($professor['firstName'] . ' ' . $professor['lastName']); ?></td>
                                    <td><?php echo htmlspecialchars($professor['email']); ?></td>
                                    <td><?php echo htmlspecialchars($professor['contactNumber']); ?></td>
                                    <td><?php echo calculateAge($professor['birthday']); ?></td>
                                    <td>
                                        <p class="<?php echo strtolower($professor['status']) === 'active' ? 'status-active' : 'status-inactive'; ?>">
                                            <?php echo htmlspecialchars($professor['status']); ?>
                                        </p>
                                    </td>
                                    <td class="buttons">
                                        <a href="edit_prof.php?userID=<?php echo encryptID($professor['userID']); ?>" class="btn-edit">
                                            <i class='bx bxs-edit'></i> 
                                        </a>
                                        <a href="archive_prof.php?userID=<?php echo encryptID($professor['userID']); ?>" class="btn-archive" onclick="return confirm('Are you sure you want to archive this professor?');">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="8">
                                    ...
                                    <?php echo $valid_age_group ? 'age ' . $valid_age_group : 'professors'; ?>
                ...
                <div class="pagination">
                    ...
                    $prev_url = "data_professor.php?" . http_build_query($query_params);
                    ?>
                    ...
                    if ($show_first) {
                        $query_params['page'] = 1;
                        $page_url = "data_professor.php?" . http_build_query($query_params);
                    ...
                    $next_url = "data_professor.php?" . http_build_query($query_params);
                    ?>
