...
try {
    $archiveSql = "UPDATE users SET archived = 1 WHERE userID = :userID AND userType = 'Professor'";
    ...
    if ($archiveStmt->execute() && $archiveStmt->rowCount() > 0) {
        $_SESSION['success_message'] = "Teacher user archived successfully!";
    } else {
        $_SESSION['error_message'] = "No teacher found with the provided ID or already archived.";
    }
} catch (PDOException $e) {
    error_log("Database Error: " . $e->getMessage());
    $_SESSION['error_message'] = "Error archiving teacher user: " . htmlspecialchars($e->getMessage());
}
