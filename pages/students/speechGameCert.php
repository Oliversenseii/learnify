...
try {
    ...
    $studentName = strtoupper(htmlspecialchars($fullName));
    $safeName = preg_replace('/[^A-Za-z0-9]/', '_', $studentName);

    $certQuery = $dbConnection->prepare("
        SELECT certificateID, completionDate 
        FROM speech_to_text_certificate 
        WHERE userID = :userID 
        LIMIT 1
    ");
    $certQuery->bindParam(':userID', $userID, PDO::PARAM_INT);
    $certQuery->execute();
    $cert = $certQuery->fetch(PDO::FETCH_ASSOC);

    if (!$cert) {
        $_SESSION['result_message'] = "Certificate not ready yet! Complete Level 30 first.";
        $_SESSION['result_status'] = 'error';
        header("Location: $redirectTo");
        exit;
    }

    $completionDate = date('F j, Y', strtotime($cert['completionDate']));

} catch (Exception $e) {
    error_log("Certificate error: " . $e->getMessage());
    $_SESSION['result_message'] = "Error loading certificate.";
    $_SESSION['result_status'] = 'error';
    header("Location: $redirectTo");
    exit;
}
...
<body>
    ...
    <div class="certificate-wrapper">
        <div class="certificate-container" id="certificate">
            <div class="certificate-border">
                ...
                <div class="certificate-content">
                    ...
                    <p>For successfully completing the Visual Speak Challenge Game up to Level 30.</p>
        ...
        <div class="btn-container">
            ...
            <button class="btn" onclick="window.location.href='imageGame.php'">Back to Game</button>
        </div>
    </div>
    ...
    <script>
        function downloadPDF() {
            ...
            html2pdf()
                .set({
                    margin: [5, 5, 5, 5],
                    filename: 'Learnify_Certificate_<?php echo $safeName; ?>_ImageGame.pdf',
