...
try {
    // Fetch current timeout duration and dashboard view
    $query = "SELECT u.password_last_changed, uts.timeout_duration, u.dashboard_view 
              FROM users u 
              LEFT JOIN user_timeout_settings uts ON u.userID = uts.userID 
    ...
    $timeout_duration = $result['timeout_duration'] ? (int)$result['timeout_duration'] : 300; // Default to 5 minutes
    $dashboard_view = $result['dashboard_view'] ?? 'table'; // Default to table
    
    // Calculate password change restriction
    $days_remaining = 0;
    if ($result && $result['password_last_changed']) {
        $last_changed = new DateTime($result['password_last_changed']);
        $now = new DateTime();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Handle auto-logout duration update
    if (isset($_POST['timeout_duration'])) {
        $new_timeout = filter_var($_POST['timeout_duration'], FILTER_VALIDATE_INT);
        if ($new_timeout !== false && $new_timeout > 0) {
            try {
                $query = "INSERT INTO user_timeout_settings (userID, timeout_duration) 
                          VALUES (:userID, :timeout_duration)
    ...
    // Handle dashboard view update
    if (isset($_POST['dashboard_view'])) {
        $new_view = $_POST['dashboard_view'] === 'card' ? 'card' : 'table';
        try {
            $query = "UPDATE users SET dashboard_view = :dashboard_view WHERE userID = :userID";
            $stmt = $dbConnection->prepare($query);
            $stmt->bindParam(':dashboard_view', $new_view, PDO::PARAM_STR);
            $stmt->bindParam(':userID', $userID, PDO::PARAM_INT);
            $stmt->execute();
            $_SESSION['dashboard_view'] = $new_view;
            $_SESSION['success_message'] = "Dashboard view updated successfully.";
            header("Location: settings.php");
            exit;
        } catch (PDOException $e) {
            error_log("Database Error: " . $e->getMessage());
            $_SESSION['error_message'] = "Failed to update dashboard view.";
        }
    }
}
        ...
        <main>
            <div class="head-title">
                <div class="left">
                    <h1>Settings</h1>
                    <ul class="breadcrumb">
                        <li><a href="./studentDash.php" style="color: var(--dark);">Home</a></li>
            ...
            <div class="settings-container">
                ...
                <div class="settings-section">
                    <div class="settings-section-header">
                        <h2><i class='bx bxs-dashboard' style="margin-right: 5px; background-color: var(--primary); color: white; padding: 7px; border-radius: 50%;"></i> Dashboard View</h2>
                        <i class='bx bx-chevron-down'></i>
                    </div>
                    <div class="settings-section-content">
                        <p>Choose how to display classes on your dashboard: Table View for a structured list or Card View for a visual, card-based layout.</p>
                        <div class="dashboard-view-form">
                            <form action="settings.php" method="post">
                                <label for="dashboard_view"><i class='bx bx-layout'></i> Dashboard View:</label><br>
                                <select name="dashboard_view" id="dashboard_view">
                                    <option value="table" <?php echo $dashboard_view === 'table' ? 'selected' : ''; ?>>Table View</option>
                                    <option value="card" <?php echo $dashboard_view === 'card' ? 'selected' : ''; ?>>Card View</option>
                                </select>
                                <button type="submit"><i class='bx bx-save'></i> Save</button>
                            </form>
                        </div>
                ...
                <div class="settings-section">
                    <div class="settings-section-header">
                        <h2><i class='bx bxs-key' style="margin-right: 5px; background-color: var(--primary); color: white; padding: 7px; border-radius: 50%;"></i> Change Password</h2>
                        <i class='bx bx-chevron-down'></i>
                    </div>
                    <div class="settings-section-content">
                        <p>Update your account password to maintain security and protect your account.</p>
                        <?php if ($days_remaining > 0): ?>
                            <p class="password-restriction">You can change your password again in <?php echo $days_remaining; ?> day<?php echo $days_remaining > 1 ? 's' : ''; ?>.</p>
                        <?php else: ?>
                            <p class="password-approved">You can change your password now.</p>
                        <?php endif; ?>
                        <button onclick="window.location.href='update_password.php'"><i class='bx bx-lock-alt'></i> Change Password</button>
                    </div>
                </div>

                <div class="settings-section">
                    <div class="settings-section-header">
                        <h2><i class='bx bxs-edit' style="margin-right: 5px; background-color: var(--primary); color: white; padding: 7px; border-radius: 50%;"></i> Edit Account</h2>
                        <i class='bx bx-chevron-down'></i>
                    </div>
                    <div class="settings-section-content">
                        <p>Modify your account details, such as username or email, to keep your information up to date.</p>
                        <button onclick="window.location.href='userAcc.php'"><i class='bx bx-user'></i> Edit Account</button>
                    </div>
                </div>
