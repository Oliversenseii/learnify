...
// Validate session
if (!isset($_SESSION['userID']) || !isset($_GET['boxID'])) {
...
try {
    ...
    // Get box details
    $boxStmt = $dbConnection->prepare("SELECT boxName FROM word_boxes WHERE boxID = :boxID AND archived = 0");
    $boxStmt->bindParam(':boxID', $boxID, PDO::PARAM_INT);
    $boxStmt->execute();
    ...
    $boxName = htmlspecialchars($box['boxName']);

    // Verify certificate exists
    $certStmt = $dbConnection->prepare("SELECT certificateID, issueDate FROM word_certificates WHERE userID = :userID AND boxID = :boxID");
    ...
    if (!$certificate) {
        $_SESSION['error_message'] = "No certificate found for this box.";
        header("Location: word_hunt.php");
        exit;
    }
    $certificateDate = date('F j, Y', strtotime($certificate['issueDate']));

    // Count completed boxes for badge
    $completedStmt = $dbConnection->prepare("SELECT COUNT(*) as completed FROM word_certificates WHERE userID = :userID");
    $completedStmt->bindParam(':userID', $userID, PDO::PARAM_INT);
    $completedStmt->execute();
    $completedBoxes = $completedStmt->fetch(PDO::FETCH_ASSOC)['completed'];
...
<body>
    <div class="certificate-wrapper">
        <div class="certificate-container" id="certificate">
            <div class="certificate-border">
                ...
                <div class="certificate-content">
                    ...
                    <p>For successfully completing the <?php echo $boxName; ?>.</p>
        ...
        <div class="btn-container">
            ...
            <button class="btn btn-back" onclick="goBack()">Back to Word Hunt</button>
        </div>
    </div>
    ...
    <script>
            ...
            window.downloadPDF = function() {
                ...
                try {
                    html2pdf()
                        .set({
                            margin: [5, 5, 5, 5],
                            filename: 'Learnify_Certificate_<?php echo $safeStudentName; ?>_<?php echo $boxID; ?>.pdf',
        ...
        function goBack() {
            window.location.href = 'word_hunt.php';
        }
    </script>
</body>
</html>