...
try {
    ...
    if ($checkStmt->rowCount() == 0) {
        $_SESSION['error_message'] = "You are not enrolled in this class.";
        header("Location: studentDash.php");
        exit;
    }

    // Get section, subject, and teacher details
    $sectionStmt = $dbConnection->prepare("
        SELECT s.sectionName, sub.subjectName, ts.teacherID, CONCAT(u.firstName, ' ', u.lastName) AS teacherName
        ...
        WHERE ts.teacherSectionID = :teacherSectionID AND ts.archived = 0 AND u.archived = 0
    ");
    $sectionStmt->bindParam(':teacherSectionID', $teacherSectionID, PDO::PARAM_INT);
    $sectionStmt->execute();
    $sectionDetails = $sectionStmt->fetch(PDO::FETCH_ASSOC);
    if (!$sectionDetails) {
        $_SESSION['error_message'] = "Class details not found.";
        header("Location: studentDash.php");
        exit;
    }
    ...
    if (isset($_POST['submitComment']) && isset($_POST['content'])) {
        ...
        header("Location: studentComments.php?teacherSectionID=$teacherSectionID");
        exit;
    }
    ...
    // Handle private message submission (to teacher only)
    if (isset($_POST['submitMessage']) && isset($_POST['messageContent'])) {
        ...
        if ($messageContent) {
            ...
            $insertMsgStmt->bindParam(':recipientID', $teacherID, PDO::PARAM_INT);
    ...
    // Fetch private messages (between student and teacher only)
    $messageStmt = $dbConnection->prepare("
        ...
        AND ((pm.senderID = :userID AND pm.recipientID = :teacherID) OR (pm.senderID = :teacherID AND pm.recipientID = :userID))
    ...
    $messageStmt->bindParam(':teacherID', $teacherID, PDO::PARAM_INT);
    $messageStmt->execute();
    ...
    // Notification logic: Fetch new private messages from teacher
    ...
    $newMessagesStmt = $dbConnection->prepare("
        ...
        JOIN users us ON pm.senderID = us.userID
        WHERE pm.teacherSectionID = :teacherSectionID AND pm.archived = 0 
        AND pm.senderID = :teacherID AND pm.recipientID = :userID AND pm.createdDate > :lastVisit
        ORDER BY pm.createdDate DESC
    ");
    $newMessagesStmt->bindParam(':teacherSectionID', $teacherSectionID, PDO::PARAM_INT);
    $newMessagesStmt->bindParam(':teacherID', $teacherID, PDO::PARAM_INT);
        ...
        <main>
            ...
            <div class="two-column-container">
                ...
                <div class="message-section">
                    <h3>Private Messages to <?php echo htmlspecialchars(ucwords($sectionDetails['teacherName'])); ?>
                        <?php if (!empty($newMessages)): ?>
                            <span class="notification-badge"><?php echo count($newMessages); ?> New</span>
                        <?php endif; ?>
                    </h3>
                    <?php if (!empty($newMessages)): ?>
                    ...
                    <div class="message-list">
                        <?php if (!empty($messages)): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p class="no">No messages available.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
